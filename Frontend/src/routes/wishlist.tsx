...
// import { PlaceCard, type Place } from "@/components/Cards";
...
export const Route = createFileRoute("/wishlist")({
  ...
  head: () => ({
    meta: [
      { title: "My Wishlist — PeoTraveller" },
      { name: "description", content: "Your saved travel destinations." },
    ],
  }),
});
...
function WishlistPage() {
  ...
  const removeMutation = useMutation({
    mutationFn: (itemId: number) => removeFromWishlist(String(itemId)),
    onSuccess: (_, itemId) => {
    ...
    <div className="min-h-screen bg-background">
            ...
            <div>
              <div className="flex items-center gap-2.5 mb-2">
                <div
                  ...
                  style={{ background: "oklch(0.590 0.195 28 / 0.10)" }}
                >
                  <BookHeart className="h-5 w-5" style={{ color: "oklch(0.590 0.195 28)" }} />
                </div>
                <h1
      ...
      <main className="container mx-auto px-4 sm:px-6 py-10">
        {isLoading ? (
          ...
          >
            Could not load your wishlist. Please sign in and try again.
          </div>
        ) : items.length === 0 ? (
          <EmptyWishlist />
        ) : (
...
function WishlistCard({
  ...
  const img = item.place?.image_url || paris;
    ...
    <article className="group relative overflow-hidden rounded-2xl bg-card border border-border/50 card-hover">
      ...
      <div className="relative aspect-[16/9] overflow-hidden">
        ...
        <button
          ...
          className="absolute top-3 right-3 flex h-8 w-8 items-center justify-center rounded-full opacity-0 group-hover:opacity-100 transition-all"
      ...
      <div className="p-4">
        <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1">
          ...
          <span>{item.place?.country || "Unknown"}</span>
        </div>
        <h3
  ...
  return (
    <div className="max-w-sm mx-auto text-center py-20 space-y-5">
      <div
        ...
        style={{ background: "oklch(0.590 0.195 28 / 0.08)" }}
      >
        <BookHeart className="h-9 w-9" style={{ color: "oklch(0.590 0.195 28 / 0.50)" }} />
      </div>
